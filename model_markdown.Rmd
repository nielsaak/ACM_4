...
long_means <- combined_param_means %>%
  pivot_longer(
    cols = -c(Subject_ID, ess_bulk, ess_tail),  # Exclude some columns from being pivoted
    names_to = "Parameter", 
    values_to = "MeanValue"
  )

# Remove columns
long_means <- long_means %>% select(-ess_bulk)
long_means <- long_means %>% select(-ess_tail)

# For some reason, we have parameter rows called 'Subject'
long_means <- long_means[long_means$Parameter != 'Subject', ]

print(head(long_means))

# Plotting
ggplot(long_means, aes(x = Parameter, y = MeanValue, fill = Subject_ID)) +
...
# Plot with facets
ggplot(long_draws_df, aes(x = .iteration, y = Value, group = .chain, color = .chain)) +
  geom_line(alpha = 0.5) +
...
ggplot(combined_data, aes(x = Value, fill = Type, alpha = Type)) +
